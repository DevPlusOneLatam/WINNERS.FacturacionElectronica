CREATE PROCEDURE IDC_FE_EXTRACCION_LINEA_IMPUESTO
(@ObjType varchar(15),@DocEntry int,@linea int )
AS BEGIN

DECLARE @MN nvarchar(3);
	
SELECT TOP 1 @MN = T0."MainCurncy"
FROM OADM T0 INNER JOIN ADM1 T1 ON T0."Code" = T1."Code";
 
 
SELECT 
MAX(A.FE_ID) "FE_ID",
MAX(A.FE_LINEAID) "FE_LINEAID",
MAX(A.FE_IMPUESTOID) "FE_IMPUESTOID",
MAX(A.FE_PORCENTAJE) "FE_PORCENTAJE",
SUM(A.FE_MONTO) "FE_MONTO",
A.FE_TIPO_TRIBUTO "FE_TIPO_TRIBUTO",
MAX(A.FE_NOMBRE) "FE_NOMBRE",
SUM(A.FE_VALOR_VENTA) "FE_VALOR_VENTA",
MAX(A.FE_ABREVIATURA) "FE_ABREVIATURA",
MAX(A.FE_CODIGO) "FE_CODIGO",
MAX(A.FE_TIPO_AFECTACION) "FE_TIPO_AFECTACION",
A.FE_MONEDA "FE_MONEDA"
FROM
 (

SELECT 
T3.U_BPP_MDTD+T3.U_BPP_MDSD+'-'+RIGHT('00000000' + Ltrim(Rtrim(T3.U_BPP_MDCD)),8) AS"FE_ID",
MAX(@linea) "FE_LINEAID",
CASE 
...
SUM(CASE T3."DocCur" WHEN @MN THEN T2.LineTotal   ELSE TotalFrgn  END) "FE_VALOR_VENTA",
CASE WHEN T2."U_IDC_TAFE" IN ('10','20','30','40') THEN T1."Rate" ELSE 0 END "FE_PORCENTAJE",
ISNULL((CASE WHEN T2."U_IDC_TAFE" IN ('10','11','12','13','14','15','16','17','20','30','40') THEN  SUM ( CASE T3."DocCur" WHEN @MN THEN T2."LineTotal"*(T2."VatPrcnt"/100)  ELSE T2."TotalFrgn"*(T2."VatPrcnt"/100) END) ELSE 0 END),0)  FE_MONTO,
CASE 
	WHEN T2."U_IDC_TAFE"='10' THEN '1000'
...
END "FE_CODIGO",
T2."U_IDC_TAFE" "FE_TIPO_AFECTACION"
FROM INV4 T0 
...
WHERE
T0."DocEntry" = @docentry  and T3."ObjType" = @ObjType AND T2."LineNum" IS NOT NULL AND (ISNULL(@linea,-1) = -1 OR T2."LineNum" = @linea) AND T2."TreeType"<>'I'
...
)A GROUP BY A.FE_TIPO_TRIBUTO ,A.FE_MONEDA;
